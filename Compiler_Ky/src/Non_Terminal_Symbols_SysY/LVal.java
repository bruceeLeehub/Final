    ...
    public static LVal analyse(IdentifySymbol identifySymbol) {
        ...
        while (identifySymbol.get_CurrentSym().getRegKey() == RegKey.LBRACK) {
            ...
            if (isRBACK) {
                ...
                Errors.add_ErrorOutPut(rowinx + " k");
            }
            in_Dims = in_Dims - 1;
        }
        ...
        if (value != null) {      // need to calculate now ,is const
            ...
            if (lenOfExpList < lenOfDims) {  // isArray
                ...
                if (size > 1 &&
                    ...
                    ArrayTableEntry arrayTableEntry = ArrayTable.getArrTable().get(Table.getAttrTableEntry(ident.getId_Name()).get_Ref());
                    int lastDim = arrayTableEntry.getUpper_Bounds().get(1);
