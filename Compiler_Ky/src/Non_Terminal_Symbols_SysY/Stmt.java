...
import Non_Terminal_Symbols_SysY.StatementPack.*;
import Parcel_My_Type.*;
import Tables.ConstStringTable;
import Word_Analyze.*;
    ...
    public static Stmt analyse(IdentifySymbol identifySymbol) {
        ...
        if (isIDENFR) {
            if (assign_Exist(identifySymbol)) {
                ...
                Errors.checkAssignValueToConst(lvalSym);
                ...
                if (judge) {
                    ...
                    if (isGETINTTK) {
                        ...
                        if (isRPARENT) {
                            ...
                            Errors.add_ErrorOutPut(rowidx + " j");
                        }
                        ...
                        if (isSEMICN) {
                            ...
                            Symbol preSymbol = identifySymbol.get_PreSym();
                            int rowidx = preSymbol.getRow_Idx();
                            Errors.add_ErrorOutPut(rowidx + " i");
                        }
                    } else {
            ...
            if (Stmt.whileStmtCount == 0 &&
                ...
                Errors.add_ErrorOutPut(rowidx + " m");
            }
            symbol = identifySymbol.getASymbol();
            regKey = symbol.getRegKey();
            //i: ';' needed
            ...
            if (regKey == RegKey.RBRACE) {
                ...
                if (FuncDef.InFuncDef && !FuncDef.haveReturnValue) {
                    ...
                    Errors.add_ErrorOutPut(rowidx + " f");
                }
                // i: ';' needed
                Symbol curSymbol = identifySymbol.get_CurrentSym();
                regKey = curSymbol.getRegKey();
            ...
            if (judge) {
                ...
                Errors.checkFormatStr(symbol, rightExpCount);
            }
            if (judge) {
                symbol = identifySymbol.getASymbol();
                ...
                if (actualExpCount != rightExpCount.my_Int) {
                    ...
                    Errors.add_ErrorOutPut(row_idx + " l");
                }
            }

            stmt = new PrintStmt(ConstStringTable.addConString(formatString), exp_List);
